...
export default function useVoiceSession() {
  ...
  const startBargeInDetection = useCallback(() => {
    ...
    bargeInCheckIntervalRef.current = setInterval(() => {
      ...
      if (rms > BARGE_IN_THRESHOLD) {
        console.log(`🛑 BARGE-IN DETECTED! (RMS: ${rms.toFixed(4)} > ${BARGE_IN_THRESHOLD})`);
        handleBargeIn();
      }
   ...
   */
  const initWebSocket = useCallback((tableId, onAudioChunk) => {
    ...
    ws.onclose = (event) => {
      ...
      if (event.code === 4004) {
        ...
        setError('Bağlantı beklenmedik şekilde kesildi. Backend çalışıyor mu?');
  ...
  const sendAudioChunk = useCallback(async (chunk) => {
    if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
      ...
      console.warn('⚠️ WebSocket not ready, chunk not sent. State:', wsRef.current?.readyState);
    }
  }, []);
